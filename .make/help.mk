...
HELP_MK_PATH:=$(dir $(abspath $(lastword $(MAKEFILE_LIST))))
HELP_LOCAL_REPO_HASH:=$(shell cd $(HELP_MK_PATH); git log -n 1 --format="%H")
HELP_REMOTE_REPO_HASH:=$(shell git ls-remote https://gitlab.com/ska-telescope/sdi/ska-cicd-makefile.git HEAD --format=%H | awk '{print $$1};')
ifneq (${HELP_LOCAL_REPO_HASH},${HELP_REMOTE_REPO_HASH})
...
long-help: ## show detailed help.
	...
	for section in $(MAKEFILE_LIST); do \
		...
		grep -E '^##.*$$' $$section | \
		awk 'BEGIN {FS = ":"} {sub(/^## ?/, ""); if (NF > 1) printf "%-9s%s\n", $$1":", $$2; else print $$0}' | \
