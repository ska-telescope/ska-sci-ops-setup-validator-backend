...
@test 'RELEASE: check .release file DOES NOT exist' {
...
@test 'RELEASE: increment release candidate - 1.1.1-rc.1' {
  cp -f pyproject.toml.default pyproject.toml
  run cd ../build && make -f ../tests/Makefile bump-rc
  run grep -E 'release=1.1.1-rc.1' ../build/.release
  assert_success
}

@test 'RELEASE: promote release candidate to release - 1.1.1' {
  cp -f pyproject.toml.default pyproject.toml
  run cd ../build && make -f ../tests/Makefile rc-to-release
  run grep -E 'release=1.1.1' ../build/.release
  assert_success
}

@test 'RELEASE: increment major release documentation - 2.0.0' {
  cp -f pyproject.toml.default pyproject.toml
  run  cd ../build && make -f ../tests/Makefile bump-major-release
  run grep -E "release = '2.0.0'" ../build/docs/src/conf.py
  assert_success
}

@test 'RELEASE: increment release candidate documentation - 2.0.0-rc.1' {
  cp -f pyproject.toml.default pyproject.toml
  run cd ../build && make -f ../tests/Makefile bump-rc
  run grep -E "release = '2.0.0rc1'" ../build/docs/src/conf.py
  assert_success
}

@test 'RELEASE: promote release candidate to release documentation - 2.0.0' {
  cp -f pyproject.toml.default pyproject.toml
  run cd ../build && make -f ../tests/Makefile rc-to-release
  run grep -E "release = '2.0.0'" ../build/docs/src/conf.py
  assert_success
